const FOOTER_COLS: { heading: string; links: [string, string][] }[] = [
  ...
  },
  {
    ...
    links: [
      ...
      ['Interview library', '#demo'],
      ['Build your own', '#library'],
      ['Coding tests', '#library'],
      ['Cognitive ability', '#library'],
      ['Personality & culture', '#library'],
      ['Language & typing', '#library'],
    ],
  },
  {
    heading: 'Solutions',
        ...
        <div className="max-w-[1240px] mx-auto">
          <div className="grid grid-cols-[1.7fr_1fr_1fr_1fr_1fr_1fr] gap-10 pb-12 border-b border-[#3A2D30] max-[900px]:grid-cols-2 max-[900px]:gap-[30px]">
            <div>
              <a href="#" className="block">
                <span className="text-[30px] font-extrabold tracking-[-0.5px] text-white">Testlify</span>
              </a>
              ...
              <div key={col.heading}>
                ...
                <div className="flex flex-col gap-3 text-[14.5px]">
                  {col.links.map(([label, href]) => (
                    <a key={label} href={href} className="text-faint hover:text-coral transition-colors">{label}</a>
                  ))}
                </div>
              </div>
            ))}
          ...
          <div className="flex items-center justify-between flex-wrap gap-4 py-6 mb-6 border-b border-[#3A2D30] max-[900px]:justify-start">
            ...
            <div className="flex items-center flex-wrap gap-3">
              {['Google for Startups', 'Microsoft for Startups', 'NVIDIA Inception', 'SHRM Labs'].map((alt) => (
                <span key={alt} className="flex items-center justify-center h-12 px-3.5 rounded-xl bg-white shadow-[0_6px_18px_rgba(0,0,0,0.16)] text-[11px] font-bold text-ink">
                  {alt}
                </span>
              ))}
            </div>
          ...
          <div className="flex justify-between items-center flex-wrap gap-4 text-[13.5px] text-muted">
            ...
            <div className="flex gap-6 flex-wrap">
              {['Privacy policy', 'Terms & conditions', 'Security', 'GDPR', 'Cookie policy'].map((label) => (
                <a key={label} href="#" className="text-muted hover:text-coral transition-colors">{label}</a>
              ))}
            </div>
          </div>
        </div>
