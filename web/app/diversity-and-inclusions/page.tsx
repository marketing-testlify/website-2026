...
.tsd-slot{display:block;width:100%;height:360px;border-radius:14px;overflow:hidden;background:#FFF0EF;}
...
const stats = ['Structured, blind assessment', 'EEOC-defensible scoring', '3,500+ validated tests'];
const logos = ['Xneelo', 'Kimp', 'Endiprev', 'Virtual Gurus', 'HROne'];

const split1 = {
  num: '1',
  h2: 'Remove bias at the source',
  body: ['Resumes carry signals — names, schools, gaps — that trigger unconscious bias. Skills-first assessment focuses on ability alone, so candidates advance on merit, not background.'],
  bullets: ['Anonymised, structured assessment', 'Consistent scoring for everyone', 'Skills over pedigree', 'Wider, fairer funnels'],
  cta: { label: 'Why it works', href: '/responsible-hiring' },
  slotId: 'tsd-slot-0',
};

const cardsSection = {
  eyebrow: 'Fair by design',
  h2: 'Inclusion built into every step',
  intro: "Fairness isn't a feature you add later — it's how the platform works.",
  cards: [
...
const split2 = {
  num: '2',
  h2: 'Find talent others miss',
  body: ['Self-taught, career-changers and non-traditional candidates often out-perform — if they get a fair shot. Skills-first hiring surfaces them.'],
  bullets: ['Surface non-traditional talent', 'Reduce screening bias', 'Improve quality of hire'],
  cta: null,
  slotId: 'tsd-slot-2',
};
...
function SplitSection({ s, bgClass, flip }: { s: Split; bgClass: string; flip: boolean }) {
  return (
    <section className={('tsd-sec ' + bgClass).trim()}>
      <div className="tsdw">
        <div className={('tsd-grid2 ' + (flip ? 'tsd-flip' : '')).trim()}>
          <div className="tsd-copy reveal">
            <span className="tsd-num">{s.num}</span>
            ...
            {s.cta && (
              <Link className="tsd-link" href={s.cta.href}>{s.cta.label}<ArrowRight /></Link>
            )}
          </div>
          <div className="tsd-media reveal">
            <div className="tsd-shot"><div className="tsd-slot" /></div>
          </div>
        </div>
      </div>
    </section>
...
export default function DiversityAndInclusionsPage() {
  ...
  return (
    <>
      ...
      <section className="tsd-hero" data-screen-label="Hero">
        <div className="tsdw tsd-hgrid">
          <div className="tsd-copy reveal">
            ...
            <p className="eyebrow">For diverse hiring<b>.</b></p>
            <h1 className="tsd-h1">Build diverse teams with bias-free, skills-first hiring</h1>
            <p className="tsd-lead">Give every applicant an equal, unbiased chance to show what they can do — and find hidden talent traditional screening filters out.</p>
        ...
        <div className="tsdw tsd-logos reveal">
          ...
          <div className="tsd-logos-r">
            {logos.map((lg) => (<span className="tsd-logo" key={lg}>{lg}</span>))}
          </div>
        </div>
      </section>

      {/* Section 1 — split */}
      <SplitSection s={split1} bgClass="" flip={false} />

      {/* Section 2 — cards */}
      <section className="tsd-sec tsd-sand">
        <div className="tsdw">
          <div className="tsd-shead reveal">
            <p className="eyebrow">{cardsSection.eyebrow}<b>.</b></p>
            <h2 className="tsd-h2">{cardsSection.h2}</h2>
            <p className="tsd-lead">{cardsSection.intro}</p>
          </div>
      ...
      <SplitSection s={split2} bgClass="" flip />

      {/* Testimonials (no data → heading only) */}
      ...
      <section className="tsd-sec tsd-sand">
        <div className="tsdw">
          ...
          <div className="tsd-atsgrid reveal">
            ...
            <Link className="tsd-atsmore" href="/integrations">+100 more<ArrowRight /></Link>
          </div>
        </div>
      </section>
      ...
      <section className="tsd-cta" id="demo">
        <div className="tsdw reveal">
          ...
          <h2 className="tsd-h2">Hire for ability, build for diversity.</h2>
          <p className="tsd-lead">Give every candidate a fair shot and find the talent others miss — start free.</p>
          <div className="tsd-ctas">
