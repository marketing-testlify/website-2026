...
export default function InterviewToOfferRatioCalculatorPage() {
  const [intv, setIntv] = useState('24');
  const [off, setOff] = useState('6');
  ...
  const nIntv = num(intv);
  const nOff = Math.max(1, num(off));
  const ratio = nIntv / nOff;
  const result = ratio.toFixed(1) + ':1';
  const rate = nIntv ? Math.round((nOff / nIntv) * 100) + '%' : '—';
  const note =
    ratio <= 4
      ...
      <section className="tsec" style={{ background: 'radial-gradient(1000px 500px at 80% 4%,#FFF0EE 0%,rgba(255,240,238,0) 60%),#fff' }}>
        <div className="tw">
          <div className="tcrumb reveal">
            ...
            <span>Interview to offer ratio calculator</span>
          </div>
          <div style={{ maxWidth: 720 }}>
            <p className="eyebrow reveal">Interview to offer ratio<b>.</b></p>
            <h1 className="th1 reveal" style={{ transitionDelay: '.04s' }}>Calculate your interview-to-offer ratio</h1>
            <p className="tlead reveal" style={{ transitionDelay: '.08s' }}>How many interviews does it take to make an offer? Enter your numbers to see the ratio — a high one usually means candidates arrive at interviews under-qualified.</p>
          </div>
        </div>
      </section>
        ...
        <div className="tw">
          <div className="tcalc reveal">
            <div className="tcard">
              ...
              <div className="tfield"><label>Interviews conducted</label><input className="tinput" type="number" min="0" value={intv} onInput={(e) => setIntv((e.target as HTMLInputElement).value)} /></div>
              <div className="tfield" style={{ marginBottom: 0 }}><label>Offers made</label><input className="tinput" type="number" min="1" value={off} onInput={(e) => setOff((e.target as HTMLInputElement).value)} /></div>
            </div>
            <div className="tresult">
              <p className="trlabel">Interview to offer ratio</p>
              <p className="trbig">{result}</p>
              <p className="trsub">{note}</p>
              <div className="trbreak">
                <div className="trrow"><span>Interviews</span><span>{intv}</span></div>
                <div className="trrow"><span>Offers</span><span>{off}</span></div>
                <div className="trrow"><span>Offer rate</span><span>{rate}</span></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="tsec">
        <div className="tw">
          <div style={{ maxWidth: 640 }}>
            ...
            <h2 className="th2 reveal" style={{ transitionDelay: '.04s' }}>The ratio, explained</h2>
            <p className="tlead reveal" style={{ transitionDelay: '.08s' }}>The interview-to-offer ratio divides interviews by offers. A lower ratio means your pipeline is well-qualified before interviews begin. Skills screening earlier is the simplest way to bring it down.</p>
          </div>
          <div className="tsteps">
            <div className="tstep reveal"><div className="tstepn">1</div><h3 className="th2" style={{ fontSize: 19, marginBottom: 8 }}>Count interviews</h3><p className="tbody" style={{ margin: 0, fontSize: 14.5 }}>Total interviews conducted over the period.</p></div>
            <div className="tstep reveal" style={{ transitionDelay: '.06s' }}><div className="tstepn">2</div><h3 className="th2" style={{ fontSize: 19, marginBottom: 8 }}>Count offers</h3><p className="tbody" style={{ margin: 0, fontSize: 14.5 }}>The number of offers you extended.</p></div>
            <div className="tstep reveal" style={{ transitionDelay: '.12s' }}><div className="tstepn">3</div><h3 className="th2" style={{ fontSize: 19, marginBottom: 8 }}>Read the ratio</h3><p className="tbody" style={{ margin: 0, fontSize: 14.5 }}>Fewer interviews per offer means a stronger pipeline.</p></div>
          </div>
        </div>
      </section>
