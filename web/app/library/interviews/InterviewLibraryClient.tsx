...
const KEYFRAMES = `
@keyframes ilib-herograd{0%{background-position:0% 0%}50%{background-position:100% 100%}100%{background-position:0% 0%}}
@keyframes ilib-blob{0%,100%{transform:translate(0,0) scale(1)}33%{transform:translate(40px,-26px) scale(1.12)}66%{transform:translate(-26px,22px) scale(.93)}}
@keyframes ilib-floaty{0%,100%{transform:translateY(0)}50%{transform:translateY(-14px)}}
@keyframes ilib-floaty2{0%,100%{transform:translateY(0)}50%{transform:translateY(12px)}}
@property --ilib-cardbang{syntax:"<angle>";initial-value:0deg;inherits:false;}
@keyframes ilib-cardrun{to{--ilib-cardbang:360deg;}}
.ilib-card{position:relative;overflow:hidden;}
.ilib-card::after{content:"";position:absolute;inset:0;border-radius:16px;padding:1.8px;background:conic-gradient(from var(--ilib-cardbang),transparent 0deg,var(--ilib-cbc,#F23F44) 55deg,transparent 150deg,transparent 360deg);-webkit-mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);-webkit-mask-composite:xor;mask:linear-gradient(#000 0 0) content-box,linear-gradient(#000 0 0);mask-composite:exclude;opacity:0;transition:opacity .35s ease;pointer-events:none;z-index:1;}
.ilib-card:hover::after{opacity:1;animation:ilib-cardrun 2.4s linear infinite;}
.ilib-cbc-beg{--ilib-cbc:#8FD9AE;}
.ilib-cbc-int{--ilib-cbc:#F4C36A;}
.ilib-cbc-adv{--ilib-cbc:#F39BA0;}
.ilib-facet-scroll::-webkit-scrollbar{width:6px;}
.ilib-facet-scroll::-webkit-scrollbar-thumb{background:#EAD7D9;border-radius:100px;}
@media(prefers-reduced-motion:reduce){
  .ilib-card::after,[style*="ilib-floaty"],[style*="ilib-blob"],[style*="ilib-herograd"]{animation:none!important;}
}
`;
...
const SearchIcon = ({ w = 20 }: { w?: number }) => (
  <svg width={w} height={w} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" aria-hidden>
...
export default function InterviewLibraryClient() {
  ...
  const data = useMemo(() => {
    ...
    let list = INTERVIEWS.filter((t) => matches(t));
    ...
    const durFacets: FacetItem[] = DUR_DEFS.map(([k, l]) => ({
      key: k,
      label: l,
      count: INTERVIEWS.filter((t) => durBucket(t.dur) === k && matches(t, "dur")).length,
      on: durs.includes(k),
  ...
  return (
    <>
      ...
      <SiteHeader
        announcement="AI video, audio and chat interviews — one platform, every role"
        announcementCta="Browse interviews"
        announcementHref={routes.libraryInterviews}
      />
      ...
      <section
        ...
        style={{
          ...
          animation: "ilib-herograd 30s ease-in-out infinite",
        }}
      >
        <div
          ...
          style={{
            ...
            animation: "ilib-blob 22s ease-in-out infinite reverse",
          }}
        />
            ...
            <Reveal delay={0.18} className="relative max-[960px]:max-w-[460px] max-[960px]:mx-auto max-[960px]:w-full">
              <div className="relative bg-white rounded-3xl shadow-[0_30px_70px_rgba(110,11,14,0.18)] p-[18px] border border-rose-250 z-[2]">
                <div className="flex items-center gap-[7px] px-0.5 pb-[14px] border-b border-[#F4E7E8] mb-[14px]">
                  ...
                  <span className="ml-[10px] text-[12px] text-faint font-medium bg-[#FBF3F3] px-3 py-1 rounded-lg flex-1">testlify.com/interviews</span>
                </div>
                ...
                <div className="flex flex-col gap-[9px]">
                  {SHOWCASE.map((s, i) => (
                    ...
                    >
                      ...
                      <span className="text-[10.5px] font-bold tracking-[0.03em] uppercase text-[#1F9D6B] bg-[#E7F6EE] px-[9px] py-1 rounded-full shrink-0">AI-scored</span>
                    </div>
                  ))}
                </div>
              </div>
              <div
                ...
                style={{ top: -26, right: 14, animation: "ilib-floaty 6s ease-in-out infinite" }}
              >
                <span
              ...
              <div
                className="absolute bg-white rounded-2xl shadow-[0_18px_40px_rgba(110,11,14,0.16)] px-[17px] py-[13px] flex items-center gap-3 border border-rose-250 z-[3]"
                style={{ bottom: -24, left: -30, animation: "ilib-floaty2 7s ease-in-out infinite" }}
              >
                ...
                <div>
                  <div className="text-[12px] text-muted font-medium">Every answer</div>
                  <div className="text-[15.5px] font-bold text-ink">AI-scored</div>
                </div>
              </div>
            </Reveal>
          </div>
            ...
            <div>
              ...
              {data.list.length > 0 ? (
                ...
                <div className="text-center py-16 px-5 border-[1.5px] border-dashed border-[#EAD7D9] rounded-[18px] bg-[#FDF7F6]">
                  ...
                  <h3 className="text-[20px] font-bold mt-3.5 mb-1.5">No interviews matched your filters</h3>
      ...
      <section className="px-7 py-10">
        <div className="max-w-[1240px] mx-auto bg-sand border border-[#F1E0DC] rounded-[22px] px-11 py-10 flex items-center justify-between gap-[30px] flex-wrap max-[760px]:px-[26px] max-[760px]:py-[30px]">
          <div>
            <h3 className="text-[24px] font-extrabold tracking-[-0.5px] m-0 mb-[7px]">Need an interview for a different role?</h3>
            <p className="text-[15px] text-body2 m-0 max-w-[520px] leading-[1.55]">
              Tell us the role and we&apos;ll generate a structured, AI-scored interview for it — questions, follow-ups and scoring criteria included.
            </p>
          </div>
          <a href="#" className="btn-sheen inline-flex items-center gap-2.5 font-bold text-[15px] rounded-xl px-6 py-3.5 bg-coral text-white shadow-[0_12px_28px_rgba(242,63,68,0.28)] hover:bg-coral-hover hover:-translate-y-0.5 hover:shadow-[0_18px_36px_rgba(242,63,68,0.36)] transition-all">
            Request an interview<ArrowRight />
          </a>
        </div>
      </section>
