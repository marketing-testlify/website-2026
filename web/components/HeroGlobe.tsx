...
/**
 * Homepage hero wireframe globe — ported 1:1 from the prototype's initGlobe()
 * (non-interactive variant: pointer-events none, auto-rotating). A 720-point
 * Fibonacci sphere with latitude rings, animated great-circle arcs and a radial
 * glow, sitting behind the candidate card. Replaces the earlier flat SVG spiral.
 */
export default function HeroGlobe() {
  ...
  useEffect(() => {
    ...
    const N = 720;
    // [x, y, z, accent] — accent (every 9th point) stored as 1/0 for typing.
    const pts: number[][] = [];
    ...
    const draw = () => {
      ...
      // auto-rotation (hero globe is non-interactive)
      yaw += 0.0026;
      ...
      for (let i = 0; i < N; i++) {
        ...
        ctx.fillStyle = pts[i][3]
          ? "rgba(247,106,110," + alpha + ")"
          : "rgba(242,63,68," + alpha + ")";
